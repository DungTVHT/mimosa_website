        ...
        public function get_pricing_html( $product ) {
            ...
            if ( $this->is_multiply_per_persons() && $product->has_people_types_enabled() ) {
                foreach ( $product->get_enabled_people_types() as $person_type ) {
                    ...
                    $price          = $this->get_price( $person_type_id );
                    if ( !$price ) {
                        $price = __( 'Free', 'yith-booking-for-woocommerce' );
                    } else {
                        $price = get_woocommerce_currency_symbol() . ' ' . number_format( $this->get_price( $person_type_id ), wc_get_price_decimals(), wc_get_price_decimal_separator(), wc_get_price_thousand_separator() );
                ...
                if ( !$price ) {
                    ...
                    $price = get_woocommerce_currency_symbol() . ' ' . number_format( $this->get_price(), wc_get_price_decimals(), wc_get_price_decimal_separator(), wc_get_price_thousand_separator() );
                    if ( $this->is_multiply_per_blocks() ) {
                        $price .= ' / ' . $product->get_block_duration_html();
                    }
                }

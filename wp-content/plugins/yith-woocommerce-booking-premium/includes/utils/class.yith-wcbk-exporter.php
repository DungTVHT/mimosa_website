...
if ( !class_exists( 'YITH_WCBK_Exporter' ) ) {
    ...
    class YITH_WCBK_Exporter {
         ...
         * @return string
         */
        public function get_ics( $post_ids, $is_admin = false ) {
            $post_ids         = !!$post_ids ? (array) $post_ids : array();
            ...
            foreach ( $post_ids as $id ) {
                ...
                if ( $booking->is_valid() ) {
                    ...
                    if ( !$booking->has_time() ) {
                        $rows[] = 'DTSTART;VALUE=DATE:' . date( $date_format, $booking->from );
                        $rows[] = 'DTEND;VALUE=DATE:' . date( $date_format, $booking->to );
                    } else {
                        $to = $booking->to;
                        if ( $booking->is_all_day() ) {
                            $to = YITH_WCBK_Date_Helper()->get_time_sum( $to, 1, 'day' );
                        }
                        $rows[] = 'DTSTART:' . date( $date_time_format, $booking->from );
                        $rows[] = 'DTEND:' . date( $date_time_format, $to );
                    }

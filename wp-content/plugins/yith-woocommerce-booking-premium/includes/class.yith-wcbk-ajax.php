...
if ( !class_exists( 'YITH_WCBK_AJAX' ) ) {
    ...
    class YITH_WCBK_AJAX {
        ...
        public function create_service() {
            ...
            $term_id     = wp_insert_term( $title, YITH_WCBK_Post_Types::$service_tax, array( 'description' => $description ) );

            if ( $term_id && is_wp_error( $term_id ) ) {
                $error_message = sprintf( __( 'Error: %s', 'yith-booking-for-woocommerce' ), $term_id->get_error_message() );
                return $this->send_json( array( 'message' => $error_message ) );
            }

            if ( $term_id ) {
                do_action( 'created_' . YITH_WCBK_Post_Types::$service_tax, $term_id );

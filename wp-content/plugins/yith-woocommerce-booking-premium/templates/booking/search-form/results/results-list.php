...
<?php if ( $products->have_posts() ) : ?>
    ...
    <?php while ( $products->have_posts() ) : $products->the_post(); ?>
        ...
        if ( !empty( $booking_request[ 'from' ] ) && !empty( $booking_request[ 'to' ] ) && !$product->has_time() ) {
            ...
            if ( $product->has_people_types_enabled() && isset( $booking_data_for_price[ 'person_types' ] ) ) {
                $booking_data_for_price[ 'person_types' ] = yith_wcbk_booking_person_types_to_list( $booking_data_for_price[ 'person_types' ] );
            }
